...
- `home/dot_config/ghostty/config` asks for a blurred background (`background-opacity = 0.85`, `background-blur = true`), which needs KWin's blur effect on (`blurEnabled` in `desktop_environment/kde/settings/kwinrc`). On KDE Plasma 6.7 KWin stopped advertising `org_kde_kwin_blur_manager` in favour of `ext_background_effect_manager_v1`, which Ghostty 1.3.1 does not speak yet, so a native Wayland window ends up transparent but unblurred. Inside a Plasma session three files start Ghostty under XWayland, where KWin still honours `_KDE_NET_WM_BLUR_BEHIND_REGION`: the wrapper `home/dot_local/bin/executable_ghostty` (every launcher here, and any shell, calls `ghostty` off `PATH`), the launcher override `home/dot_local/share/applications/com.mitchellh.ghostty.desktop`, and the systemd drop-in `home/dot_config/systemd/user/app-com.mitchellh.ghostty.service.d/gdk-backend.conf` for the D-Bus activated instance. Drop all three once a Ghostty release speaks the new protocol.
