...
- `gnome/extensions.dconf` and `gnome/extensions.yaml` are the original owner's captured GNOME Shell extension set. Inside a GNOME session the Ansible `gnome` role installs and applies them automatically. Delete `gnome/extensions.dconf` or re-capture with `./scripts/gnome-extensions-sync.sh capture` before your first run.
