...
- a failing phase is recorded and skipped in best-effort mode and recorded once and fatal in strict mode, and the Linux package installer installs in one transaction in strict mode, one package at a time in best-effort and low-memory mode, reporting `changed` only for packages the package manager acted on (`test/setup_mode_phases.sh`)
...
- GNOME extension capture drops runtime keys and uninstalled UUIDs and is stable across runs, `check` and `diff` see setting, extension and enabled-list drift, and `apply` loads settings before enabling what is installed (`test/gnome_extensions_sync.sh`)
