...
The GitHub add-ons resolve their release through the `releases/latest` redirect and download assets by name, using the GitHub API only as a fallback: the unauthenticated API allows 60 requests an hour per address, and a few bootstrap runs from one network exhausted it, leaving the geometry change effect silently unresolved. After each install the script checks that the package's own metadata reports the expected version, copies the package into place itself if `kpackagetool6` fails or is missing, and prints the tool's error so the setup report says why. It then loads the geometry change effect into the running KWin over D-Bus and reports whether KWin has it; when KWin lists the effect but refuses to load it, the reason is almost always software rendering (a `QPainter` compositor, typical of a VM without 3D acceleration), which no install step can fix, so the script names the compositing type and says so, and the per-add-on result lines are carried into the setup report through `dotfiles_feature_details`, so a report from another machine shows what each add-on did rather than just that the role finished.
